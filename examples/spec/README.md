...
`task46-regressions.py` 提交 152 个 schema-relative complete-root probes：catalog exact
...
diagnostic。再加十二个完整根要求 empty Prompt table 不得放行 nested
...
container 全部收口为 stable diagnostic。最后一正八负完整根把同一 declaration
Prompt scope 贯穿 LatentSite 与 path-level `DemandV1` / `RequestV1`，禁止后两处使用
Kernel-Forward-only `OuterOfV1`，并要求 demand/request/site 的 canonical
attribution key 一致；同时 exact-decode suspension atoms list，并从 atoms
重新计算 aggregate grade。再以三正七负完整根冻结 secondary template 与
`LatentSite.secondary_sites` sealed evidence 的 exact 一致、普通 Function 的
site/request exact closure，以及 Handler 只接受有 Call projection、当前 exact
handler removal 或 Forward disposition 证明的 finite extra site；Forward 证明还必须
匹配 derived HandlerInstall stage并逐字段匹配 LatentSite 的完整语义投影，不能只借用
五字段 attribution key或无 Call evidence地改报 Call stage。主 validator
同样以独立进程运行本 gate。
